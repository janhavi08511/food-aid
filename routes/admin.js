...
router.post("/admin/donation/assign/:donationId", middleware.ensureAdminLoggedIn, async (req,res) => {
	try
	{
        ...
        // Explicitly cast the agent string to a MongoDB ObjectId
        const agentId = new mongoose.Types.ObjectId(agent); // <--- ADD THIS LINE

		await Donation.findByIdAndUpdate(donationId, { status: "assigned", agent: agentId, adminToAgentMsg }); // <--- USE agentId HERE
